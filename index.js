	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔️ Prontinho aki sua figurinha ✔️',
				error: {
			...
			switch(command) {
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply('❌ Falha, ao converte ${tipe} o stiker')
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						await ffmpeg(`./${media}`)
							...
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(412,iw)':min'(412,ih)':force_original_aspect_ratio=decrease,fps=15, pad=412:412:-1:-1:color=${args[0]}@0.0, split [a][b]; [a] palettegen=reserve_transparent=off; [b][p] paletteuse`])
							.toFormat('webp')
							.save(ran)*/
					} else {
				...
				case 'yt2mp3':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('ta procurando oque meu consagrado?')
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, BANIDOKKKKKKKKKKKKK :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('TU NEM E ADM POW')
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('O modo Simi está ativado')
						samih.push(from)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Ativar com sucesso o modo Bot chato neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desativando o modo bot chato com sucesso neste grupo ✔️')
					} else {
